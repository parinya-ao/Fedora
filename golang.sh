...
rm -f "$tempfile"
# ใช้ curl พร้อมแถบความคืบหน้า และใช้ progress_bar แสดงผล
curl -L "$url" -o "$tempfile" --progress-bar 2>&1 | while read -r line; do
  # จับเปอร์เซ็นต์ความคืบหน้าจาก output ของ curl (มองหารูปแบบ XX%)
